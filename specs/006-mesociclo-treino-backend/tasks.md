...
- [X] T008 Aplicar a migração localmente (`supabase migration up`) e confirmar que a tabela `mesocycle_plans` foi criada — **verificado 2026-09-26** com Supabase local (CLI 2.118.0 via `npx supabase`, Docker em container): `supabase start` aplicou as 7 migrations em ordem, `supabase migration up` retornou nada pendente e `migration list --local` mostra local = remoto. `\d mesocycle_plans` confere com `data-model.md`: colunas/defaults, FK `user_id → auth.users ON DELETE CASCADE`, índice `idx_mesocycle_plans_user_created (user_id, created_at DESC)`, RLS ativa com a policy `auth.uid() = user_id`
...
- [X] T047 Seguir o fluxo manual de `quickstart.md` de ponta a ponta (bootstrap, ajuste por recovery em dois cenários, pular dias, completar o ciclo inteiro e confirmar regeneração automática) — **verificado 2026-09-26** contra a API real (Fastify, `tsx src/index.ts`) + Supabase local (auth real via signup, RLS ativa). Única peça simulada: a API da Anthropic, trocada por um servidor HTTP local via `ANTHROPIC_BASE_URL` que devolve um mesociclo fixo de 4 sessões (sem chave de API no ambiente). Resultados: (1) bootstrap — 1º `GET /workout/latest-plan` devolve `status: 'generating'` e dispara a geração; o 2º devolve a sessão A, `upcoming` BCD, progresso 0/4, 1 linha em `mesocycle_plans`; (2) ajuste no mesmo dia sem novo insert — HRV 75 → sem ajuste (4x80kg, 3x150kg); HRV 30 → `adjusted: true`, 3x72kg / 2x135kg ("exausto"); HRV 50 → 3x80kg / 2x150kg ("cansado"); sempre 1 linha no banco (a regra de ajuste atual é a da spec 011, por níveis); (4) pular dias — com o mesociclo retroagido em 5 dias, `today` continua sendo a sessão A; (3) ciclo completo — 3 POSTs em `/api/workouts/sessions` avançam B → C → D (1/4, 2/4, 3/4) sem chamar a IA; o 4º marca o ciclo 4/4 e gera um novo `mesocycle_plans` (2ª chamada à IA), e o `GET` seguinte já devolve o novo ciclo em A, 0/4. Log da API sem nenhum erro
